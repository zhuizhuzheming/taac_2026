"""PCVR Parquet dataset module (v9.2-fixed — Anti-Leak + Correct CVR Label Logic).

Fixes:
1. Train/valid unified click-filtering + CVR label mapping (fixes valid_AUC=0).
2. Pre-built col_name map for robust OOB vocab limiting (fixes offset-vs-index bug).
3. _flush_buffer correctly merges list fields (fixes user_id truncation).
4. Validation sequence truncation clips to max_len (fixes potential OOB).
"""
...
class PCVRParquetDataset(IterableDataset):
    ...
    v9.2-fixed:
    - Unified click-filtering for both train and valid (CVR task semantics).
    - Pre-built col_name map for OOB vocab limiting.
    - Safe list merging in buffer flush.
    - Clipped sequence truncation.
    """
    ...
    def __init__(
        ...
        # 【v9.2-fix】预构建 (group, col_idx) -> col_name 映射，避免 _record_oob 中误用 offset 当索引
        ...
        logging.info(
            f"PCVRParquetDataset v9.2-fixed: {self.num_rows} rows, "
        ...
        if not self.is_training and self._train_vocab is not None:
            ...
            if col_name and col_name in self._train_vocab:
                ...
                if effective_vocab < vocab_size:
                    logging.debug(f"【v9.2-anti-leak】Limiting vocab {col_name}: {vocab_size} -> {effective_vocab}")
    ...
    def _pad_varlen_int_column(
        ...
        """【v9.2-opt】向量化版本：用NumPy高级索引替代Python循环"""
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Optional[Dict[str, Any]]:
        ...
        v9.2-fixed:
        - Unified click-filtering for CVR: drop non-click rows, label=(label_type==2).
        """
        B = batch.num_rows
        ...
        # 【v9.2-fix】CVR 任务统一逻辑：过滤未点击样本，点击转化=1，点击未转化=0
        ...
        if self.is_training:
            ...
            # 验证/测试时：正常保留点击样本；若 label 被隐藏（全 0）则保留全部
            if click_mask.any():
                labels = (label_types == 2).astype(np.int64)
                n_keep = int(click_mask.sum())
            else:
                ...
                for i in range(B):
                    ...
                    if end > start:
                        ...
                        if len(valid_positions) > 0:
                            # 【v9.2-fix】限制 new_len 不超过 max_len
