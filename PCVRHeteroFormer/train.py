"""HeteroFormer training entry point — v11.2 (Hybrid: RoPE + SwiGLU + Top-K + Explicit Interaction)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="HeteroFormer Training v11.2")
    ...
    # v10 参数：保留以兼容 run.sh，但不再传给 model
    ...
    parser.add_argument('--ohem_ratio', type=float, default=1.0)
    parser.add_argument('--curriculum_epochs', type=int, default=3)
    parser.add_argument('--early_exit_weight', type=float, default=0.3)

    # v11.2 NEW parameters (HyFormer graft)
    parser.add_argument('--seq_top_k', type=int, default=0,
                        help='Top-k sequence compression (0=disabled). '
                             'Enable for long sequences >1k to reduce O(N^2) attention cost.')
    parser.add_argument('--rope_base', type=float, default=10000.0,
                        help='RoPE base frequency. Lower values emphasize local patterns.')
    ...
    # v11.2: env override for seq_top_k (useful for A/B testing without changing run.sh)
    env_top_k = os.environ.get('SEQ_TOP_K', '')
    if env_top_k:
        try:
            args.seq_top_k = int(env_top_k)
        except ValueError:
            pass
...
def main() -> None:
    ...
    # v11.2: Model args with seq_top_k and rope_base
    model_args = {
    ...
    logging.info(f"PCVRHeteroFormer v11.2 (Hybrid: RoPE + SwiGLU + Top-K + Explicit Interaction)")
    logging.info(f"Total parameters: {total_params:,} | Embedding: {emb_params:,} | Dense: {dense_params:,}")
    if args.seq_top_k > 0:
        logging.info(f"Sequence Top-K compression enabled: k={args.seq_top_k}")
    ...
    trainer = PCVRHeteroFormerTrainer(
        ...
        early_exit_weight=args.early_exit_weight,
        seq_max_lens=seq_max_lens,
    )
