...
# ═══════════════════════════════════════════════════════════════════════════════
# TokenMixer-Large Components (NEW) — with surgical modifications
# ═══════════════════════════════════════════════════════════════════════════════
...
class RMSNorm(nn.Module):
    ...
    TokenMixer-Large (A.4) 使用 RMSNorm 替代 LayerNorm 以降低计算开销，
    并移除所有 bias（因为归一化后 bias 数学上冗余）。
    """
...
class TokenWiseSwiGLU(nn.Module):
    """参数隔离的 Per-token SwiGLU（TokenMixer-Large 3.3.2），已做手术修改。

    修改点（手术三）：
    - 关闭 small init down (use_small_init_down=False)，加速收敛。
    - 提高 dropout 防止过拟合。
    """
    ...
    def __init__(
        ...
        hidden_mult: int = 4,
        dropout: float = 0.0,
        use_small_init_down: bool = False,  # 改为默认 False
    ) -> None:
        super().__init__()
        ...
        # Per-token 参数: (num_tokens, dim, hidden_dim) / (num_tokens, hidden_dim, dim)
        self.up_projs = nn.Parameter(torch.empty(num_tokens, dim, self.hidden_dim))
        self.gate_projs = nn.Parameter(torch.empty(num_tokens, dim, self.hidden_dim))
        self.down_projs = nn.Parameter(torch.empty(num_tokens, self.hidden_dim, dim))
        self.dropout = nn.Dropout(dropout)

        # 初始化
        for t in range(num_tokens):
            nn.init.xavier_uniform_(self.up_projs[t])
            nn.init.xavier_uniform_(self.gate_projs[t])
            if use_small_init_down:
                nn.init.xavier_uniform_(self.down_projs[t], gain=0.01)
            else:
                nn.init.xavier_uniform_(self.down_projs[t])

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, num_tokens, dim)
        up = torch.einsum('btd,tde->bte', x, self.up_projs)
        gate = torch.einsum('btd,tde->bte', x, self.gate_projs)
        hidden = F.silu(gate) * up          # Swish = SiLU
        hidden = self.dropout(hidden)
        out = torch.einsum('bte,ted->btd', hidden, self.down_projs)
        return out


class TokenMixerLargeBlock(nn.Module):
    """TokenMixer-Large Block（3.3），经过手术改造。

    手术一：Sub-layer 1 的 per-token SwiGLU 替换为共享参数的 SwiGLU FFN，
            大幅削减参数，同时保留 Pre-Norm + Mixing/Reverting 结构优势。
    手术三：关闭 small init，提高 dropout（由外部传入）。
    其余部分（Sub-layer 2 的 per-token SwiGLU、RMSNorm、Pre-Norm）完全保留。
    """
    ...
    def __init__(
        ...
        dropout: float = 0.0,
        mode: str = 'full',
        num_mix_tokens: Optional[int] = None,
    ) -> None:
        super().__init__()
        self.T = n_total
        self.D = d_model
        self.H = num_mix_tokens if num_mix_tokens is not None else n_total
        self.mode = mode
        ...
        if mode == 'full':
            if d_model % self.H != 0:
                raise ValueError(
                    f"d_model={d_model} must be divisible by H={self.H} for token mixing."
                )
            self.d_sub = d_model // self.H
            self.mixed_dim = self.T * self.d_sub   # 每个 mixed token 的维度

        # Pre-Norm RMSNorm（A.4）
        self.norm1 = RMSNorm(d_model)
        self.norm2 = RMSNorm(d_model)

        # 手术一：Sub-layer 1 改为共享 SwiGLU FFN（无 bias，与 RMSNorm 风格一致）
        if mode == 'full':
            self.shared_ffn1 = nn.Sequential(
                nn.Linear(self.mixed_dim, self.mixed_dim * hidden_mult, bias=False),
                nn.SiLU(),
                nn.Dropout(dropout),
                nn.Linear(self.mixed_dim * hidden_mult, self.mixed_dim, bias=False),
                nn.Dropout(dropout),
            )
            # 下投影用小 gain 保持稳定性（但不再使用极小的 0.01）
            nn.init.xavier_uniform_(self.shared_ffn1[-2].weight, gain=0.1)
        else:
            self.shared_ffn1 = None

        # Sub-layer 2：保留 per-token SwiGLU（手术三参数已修改默认值）
        self.swiglu2 = TokenWiseSwiGLU(
            num_tokens=self.T,
            dim=d_model,
            hidden_mult=hidden_mult,
            dropout=dropout,
            use_small_init_down=False,  # 关闭 small init
        )

    def mixing(self, Q: torch.Tensor) -> torch.Tensor:
        """(B, T, D) -> (B, H, T*D/H)"""
        B, T, D = Q.shape
        Q_split = Q.view(B, T, self.H, self.d_sub)
        Q_trans = Q_split.transpose(1, 2).contiguous()
        Q_hat = Q_trans.view(B, self.H, T * self.d_sub)
        return Q_hat

    def reverting(self, H: torch.Tensor) -> torch.Tensor:
        """(B, H, T*D/H) -> (B, T, D)"""
        B, H, TDH = H.shape
        H_reshaped = H.view(B, H, self.T, self.d_sub)
        H_trans = H_reshaped.transpose(1, 2).contiguous()
        X_revert = H_trans.view(B, self.T, self.D)
        return X_revert
    ...
    def forward(self, Q: torch.Tensor) -> torch.Tensor:
        ...
        # === First sub-layer: Mixing + 共享 SwiGLU + Reverting + Residual ===
        Q_norm = self.norm1(Q)              # Pre-Norm

        if self.mode == 'full':
            Q_mixed = self.mixing(Q_norm)   # (B, H, T*D/H)
            Q_mixed = self.shared_ffn1(Q_mixed)   # 共享 FFN
            Q_revert = self.reverting(Q_mixed)    # (B, T, D)
        else:
            # ffn_only fallback：无 mixing，直接用 per-token SwiGLU
            Q_revert = self.swiglu2(Q_norm)

        # Residual：语义对齐
        Q_out = Q + Q_revert

        # === Second sub-layer: Per-token SwiGLU + Residual ===
        Q_norm2 = self.norm2(Q_out)
        Q_ffn = self.swiglu2(Q_norm2)       # (B, T, D)
        Q_out = Q_out + Q_ffn
...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    def __init__(
        ...
        mixer_type: str = 'tokenmixer_large',   # 'rankmixer' | 'tokenmixer_large'
    ) -> None:
        super().__init__()
        self.num_sequences = num_sequences
        self.num_queries = num_queries
        ...
        # RankMixer / TokenMixer-Large 选择
        n_total = num_queries * num_sequences + num_ns
        if mixer_type == 'rankmixer':
        ...
        elif mixer_type == 'tokenmixer_large':
            # TokenMixer-Large 固定使用 full mixing+reverting 模式（已手术改造）
            self.mixer = TokenMixerLargeBlock(
                d_model=d_model,
                n_total=n_total,
                hidden_mult=hidden_mult,
                dropout=dropout,
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        num_heads: int = 4,
        seq_encoder_type: str = 'longer',      # 改动：默认使用 longer，配合 RoPE
        hidden_mult: int = 2,                  # 改动：降低至 2，防止过拟合
        dropout_rate: float = 0.05,            # 改动：提高至 0.05
        seq_top_k: int = 30,                   # 改动：适合序列长度
        seq_causal: bool = False,
        action_num: int = 1,
        num_time_buckets: int = 65,
        rank_mixer_mode: str = 'full',
        mixer_type: str = 'tokenmixer_large',  # 改动：默认使用手术后的 TokenMixer
        use_rope: bool = True,                 # 改动：开启 RoPE
        rope_base: float = 10000.0,
        emb_skip_threshold: int = 0,
        seq_id_threshold: int = 10000,
        # NS tokenizer variant
        ns_tokenizer_type: str = 'group',      # 改动：使用 group 模式，语义更稳定
